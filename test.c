//单元测试确保其他人修改代码后，原来的功能维持正确（这称为回归测试）。
//常用的单元测试框架有 xUnit 系列，如 C++ 的 Google Test、C# 的 NUnit。
//一般来说，软件开发是以周期进行的。例如，加入一个功能，再写关于该功能的单元测试。
//但也有另一种软件开发方法论，称为测试驱动开发TDD，它的主要循环步骤是：
//1.加入一个测试。
//2.编写、修改实现代码并运行所有测试，直到成功
//3.重构代码，加入新测试
//TDD 是先写测试再实现。好处是实现会刚好满足测试，而不会写了一些不需要的代码，或是没有被测试的代码。
//但无论我们是采用 TDD，或是先实现后测试，都应尽量加入足够覆盖率的单元测试。
//完全按照 TDD 的步骤来开发，会减慢开发进程。建议在这两种极端的工作方式取平衡。通常会在设计 API 后，先写部分测试再实现。
...
//如果 expect != actual（预期值!=实际值），便会输出错误信息
...
//lept_parse() 只返回 LEPT_PARSE_OK：
//第一个返回 LEPT_PARSE_OK，所以是通过的。第二个测试因为 lept_parse() 没有把 v.type 改成 LEPT_NULL，造成失败。
//我们再实现 lept_parse() ，通过测试。
...
static void test_parse_string() {//字符串测试
    TEST_STRING("", "\"\"");
...
static void test_parse_array() {//数组测试
    size_t i, j;
    lept_value v;
    ...
    EXPECT_EQ_SIZE_T(0, lept_get_array_size(&v));//验证 lept_get_array_size() 返回值是否正确
    lept_free(&v);
...
static void test_stringify() {//检测JSON生成器
    //把一个 JSON 解析，然后再生成另一 JSON，逐字符比较两个 JSON 是否一模一样。这种测试称为往返测试
    TEST_ROUNDTRIP("null");
...
//应用方的代码在调用 lept_parse() 之后，最终也应该调用 lept_free() 去释放内存。
//我们把之前的单元测试也加入此调用。
